...
class BangunanBangunan(zona.BangunanZona):
    ...
    zona_id = fields.Many2one(
        comodel_name='bangunan.zona',
        string='Bangunan Zona ID'
    )
    ...
    jenis_bangunan = fields.Selection([
        ...
        ('ibadah', 'Ibadah'),
        ('sosial', 'Sosial'),
    ], string='Bangunan Jenis Bangunan',
        default='kantor'
    )

    nomor_imb = fields.Text(
        string='Bangunan Nomor IMB'
    )

    alamat = fields.Text(
        string='Bangunan Alamat'
    )

    sertifikat = fields.Selection([
        ('shm', 'SHM'),
        ('shgb', 'SHGN'),
    ], string='Bangunan Sertifikat',
        default='shm'
    )

    nomor_sertifikat = fields.Text(
        string='Bangunan Nomor Sertifikat'
    )

    keterangan = fields.Text(
        string='Bangunan Keterangan'
    )

    lantai_count = fields.Integer(
        string='Lantai',
        compute='get_lantai_count'
    )
